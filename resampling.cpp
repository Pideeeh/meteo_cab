...
int main() {
    ...
    //---------------------------------
    reader->SetFileName(getDataPath(filename + ".vti").c_str());//adapt
    //--------------------------
    reader->Update();
    ...
    // this is dataset specific
    //---------------------------------
    vtkSmartPointer<vtkAbstractArray> arr = pts->GetAbstractArray(0);
    string array_name = arr->GetName();
    //----------------------------------
    ...
    // this is dataset specific
    //---------------------------------
    vtkSmartPointer<vtkAbstractArray> arr_reg = pts_reg->GetAbstractArray(0);
    arr_reg->SetName(array_name.c_str());


    ifstream myfile;
    myfile.open(
            getDataPath("/data/height.txt").c_str());//For wa, adapt to height2.txt. For all others, adapt to height.txt
    string str;
    ...
    for (int y = 0; y < new_dims[1]; y++) {
        for (int x = 0; x < new_dims[0]; x++) {
            ...
            for (int z = 0; z < new_dims[2]; z++) {
                double regular_sample_in_data_space =
                        (z) * (lower_height - upper_height) / resolution + upper_height - terrain_height;
                int upper, lower;
                double coeff;
